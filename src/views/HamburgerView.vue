<template>
  <div class="container-fluid">
    <button
      ...
      @click="addClick()"
    >
      Hamburger hozzáadása
    </button>
    <div class="joob">
      <div class="d-flex d-row">
        <input
          class="form-control m-2"
          v-model="HamburgerNevFilter"
          v-on:keyup="FilterFn()"
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="input-group mb-3">
              ...
              <input type="text" class="form-control" v-model="HamburgerNev" />
            </div>
            <div class="input-group mb-3">
              <span class="input-group-text">Ár</span>
              <input type="number" class="form-control" v-model="HamburgerAr" />
            </div>
            <div class="input-group mb-3">
            ...
            <div class="p2 w50 bd-highlight">
              <img
                ...
                width="460px"
                height="290px"
                :src="PhotoPath + HamburgerLinkkep"
              />
              <input
                v-if="logged"
                class="m2"
            ...
            <button
              type="button"
              @click="adatbazisbaTeszClick(HamburgerId, Kategoria, aktDarab)"
...
export default {
  ...
  methods: {
    refreshData() {
      axios.get(this.$store.state.API_URL + "termek/3").then((response) => {
        this.termekek = response.data;
        this.TermekekSzuroNelkul = response.data;
      });
    },
    KosarClick(termek) {
      this.HamburgerId = termek.id;
      this.HamburgerNev = termek.nev;
      this.HamburgerAr = termek.ar;
      this.HamburgerLinkkep = termek.linkkep;
    ...
    addClick() {
      this.modalTitle = "Hamburger hozzáadása";
      this.HamburgerId = 0;
      this.HamburgerNev = "";
      this.HamburgerAr = "";
      this.HamburgerLinkkep = "";
      this.Kategoria = 5;
    },
    editClick(termek) {
      this.modalTitle = "Hamburger módosítása";
      this.HamburgerId = termek.id;
      this.HamburgerNev = termek.nev;
      this.HamburgerAr = termek.ar;
      this.HamburgerLinkkep = termek.linkkep;
    ...
    imageUpload(event) {
      ...
      axios
        ...
        .then((response) => {
          this.HamburgerLinkkep = "/img/" + response.data;
        });
    },
    createClick() {
      axios
        .post(
          ...
          {
            Nev: this.HamburgerNev,
            Ar: this.HamburgerAr,
            Linkkep: this.HamburgerLinkkep,
            Kategoria: 5,
          }
        )
        .then((response) => {
    ...
    FilterFn() {
      var HamburgerNevFilter = this.HamburgerNevFilter;
      this.termekek = this.TermekekSzuroNelkul.filter(function (el) {
        console.log(el);
        return el.nev
          .toString()
          .toLowerCase()
          .includes(HamburgerNevFilter.toString().trim().toLowerCase());
      });
    },
  },
  mounted: function () {
