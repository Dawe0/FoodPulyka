<template>
  <div class="container-fluid">
    <button
      ...
      @click="addClick()"
    >
      SzeszesItal hozzáadása
    </button>
    <div class="joob">
      <div class="d-flex d-row">
        <input
          class="form-control m-2"
          v-model="SzeszesItalNevFilter"
          v-on:keyup="FilterFn()"
          ...
          <div class="modal-body">
            ...
            <div class="p2 w50 bd-highlight">
              <img
                ...
                width="460px"
                height="290px"
                :src="PhotoPath + SzeszesItalLinkkep"
              />
              <input
                v-if="logged"
                class="m2"
            ...
            <button
              type="button"
              @click="adatbazisbaTeszClick(SzeszesItalId, Kategoria, aktDarab)"
...
export default {
  ...
  methods: {
    refreshData() {
      axios.get(this.$store.state.API_URL + "termek/6").then((response) => {
        this.termekek = response.data;
        this.TermekekSzuroNelkul = response.data;
      });
    },
    KosarClick(termek) {
      this.SzeszesItalId = termek.id;
      this.SzeszesItalNev = termek.nev;
      this.SzeszesItalAr = termek.ar;
      this.SzeszesItalLinkkep = termek.linkkep;
    ...
    addClick() {
      this.modalTitle = "SzeszesItal hozzáadása";
      this.SzeszesItalId = 0;
      this.SzeszesItalNev = "";
      this.SzeszesItalAr = "";
      this.SzeszesItalLinkkep = "";
      this.Kategoria = 5;
    },
    editClick(termek) {
      this.modalTitle = "SzeszesItal módosítása";
      this.SzeszesItalId = termek.id;
      this.SzeszesItalNev = termek.nev;
      this.SzeszesItalAr = termek.ar;
      this.SzeszesItalLinkkep = termek.linkkep;
    ...
    imageUpload(event) {
      ...
      axios
        ...
        .then((response) => {
          this.SzeszesItalLinkkep = "/img/" + response.data;
        });
    },
    createClick() {
      axios
        .post(
          ...
          {
            Nev: this.SzeszesItalNev,
            Ar: this.SzeszesItalAr,
            Linkkep: this.SzeszesItalLinkkep,
            Kategoria: 5,
          }
        )
        .then((response) => {
    ...
    FilterFn() {
      var SzeszesItalNevFilter = this.SzeszesItalNevFilter;
      this.termekek = this.TermekekSzuroNelkul.filter(function (el) {
        console.log(el);
        return el.nev
          .toString()
          .toLowerCase()
          .includes(SzeszesItalNevFilter.toString().trim().toLowerCase());
      });
    },
  },
  mounted: function () {
